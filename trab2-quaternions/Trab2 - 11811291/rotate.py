...
class Cube(): #classe Cubo
    ...
    def __str__(self): #método str para a classe cube
        s = "[" + str(round(self.x, 6)) +" , "+ str(round(self.y, 6)) +" , "+ str(round(self.z, 6))+"]"
        return s
    ...
    def findR(self, alpha): #função para encontrar R, sem poluir a função rotate
        ...
        r.q = math.cos(ang)
        r.i = (math.sin(ang))**(1.0/normaV)*self[0]
        r.j =  (math.sin(ang))**(1.0/normaV)*self[1]
        r.k =  (math.sin(ang))**(1.0/normaV)*self[2]
        return r   
