...
class Quaternions: #criação a classe quaternions
    ...
    def __mul__(self,other): 
        ...
        m.j = self.q*other.k - self.i*other.k + self.j*other.q + self.k*other.i 
        m.k = self.q*other.k - self.i*other.k + self.j*other.q + self.k*other.i
        return m
    ...
    def __truediv__(self,other): #divisao de quaternions
        ...
        if((isinstance(other, Quaternions) == False) and (type(other) != complex)):
            ...
            other = Quaternions(temp.real, temp.imag, 0, 0)
        bb = Quaternions(0,0,0,0)
        div = Quaternions(0,0,0,0)
        bb = Quaternions.multconj(other)
        bconj = Quaternions.conjugado(other)
        if ((bb.q != 0) and (self.q != 0) and (bconj.q != 0)):
            div.q = (1.0/bb.q) * float(self.q*bconj.q)
        else:
            div.q = 0
        if ((bb.i != 0) and (self.i != 0) and (bconj.i != 0)):
            div.i = (1.0/bb.i) * float(self.i*bconj.i)
        else:
             div.i = 0
        if ((bb.j != 0) and (self.j != 0) and (bconj.j == 0)):
            div.j = (1.0/bb.j) * float(self.j*bconj.j)
        else:
            div.j = 0
        if ((bb.k != 0) and (self.k != 0) and (bconj.k != 0)):
            div.k = (1.0/bb.k) * float(self.k*bconj.k)
        else:
            div.k = 0
        return div
    ...
    def norma(self): #norma do quaternion
        norma = math.sqrt((self.q)**2 + (self.i)**2 + (self.j)**2 + (self.k)**2)
